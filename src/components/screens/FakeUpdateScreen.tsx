  ...
  switch (theme) {
    case 'windows-10':
      return (
        ...
        >
          <div className="mac-spinner" style={{ width: isPreview ? '36px' : '50px', height: isPreview ? '36px' : '50px', border: '4px solid rgba(255,255,255,0.2)', borderTopColor: '#ffffff', marginBottom: isPreview ? '1.25rem' : '2.5rem' }}></div>
          <div style={{ textAlign: 'center', maxWidth: '600px', width: '100%' }}>
            <h2 style={{ fontSize: isPreview ? '1.15rem' : '1.75rem', fontWeight: 300, marginBottom: '0.75rem', lineHeight: 1.3 }}>
              {title || 'Trabajando en las actualizaciones'} {displayPercent}%
            </h2>
            <p style={{ fontSize: isPreview ? '0.82rem' : '1rem', opacity: 0.85, lineHeight: 1.4 }}>
              {message || 'No apagues el equipo. Esto tardará unos minutos.'}
            </p>
            <p style={{ fontSize: isPreview ? '0.75rem' : '0.85rem', opacity: 0.6, marginTop: isPreview ? '1rem' : '1.5rem' }}>
    ...
    case 'macos':
      return (
        ...
        >
          ...
          <svg
            viewBox="0 0 170 170"
            style={{ width: isPreview ? '48px' : '80px', height: isPreview ? '48px' : '80px', fill: '#ffffff', marginBottom: isPreview ? '1.5rem' : '4rem' }}
          >
          ...
          <div style={{ fontSize: isPreview ? '0.75rem' : '0.8rem', opacity: 0.5, marginTop: '0.4rem', textAlign: 'center' }}>
        ...
        >
          <div
            style={{
              ...
              marginBottom: isPreview ? '1.5rem' : '3rem',
            }}
          ></div>
          
          <div style={{ textAlign: 'left', width: '100%', maxWidth: '400px' }}>
            <h3 style={{ fontSize: isPreview ? '1rem' : '1.2rem', fontWeight: 400, marginBottom: '0.4rem', color: '#f3f4f6' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: isPreview ? '0.75rem' : '0.85rem', color: '#9ca3af' }}>
              <span>{message || 'Instalando paquete...'}</span>
    ...
    default:
      return (
        ...
        >
          ...
          >
            {[...Array(6)].map((_, i) => (
              <div
                ...
                style={{
                  position: 'absolute',
                  width: isPreview ? '5px' : '6px',
                  height: isPreview ? '5px' : '6px',
                  borderRadius: '50%',
                  backgroundColor: '#ffffff',
                  left: isPreview ? '17px' : '25px',
                  top: isPreview ? '17px' : '25px',
                  transformOrigin: isPreview ? '3px 20px' : '3px 28px',
          ...
          <div style={{ textAlign: 'center', maxWidth: '600px', width: '100%' }}>
            <h2 style={{ fontSize: isPreview ? '1.15rem' : '1.5rem', fontWeight: 300, marginBottom: '0.6rem', letterSpacing: '0.5px' }}>
              {title || 'Actualizando el equipo'} {displayPercent}%
            </h2>
            <p style={{ fontSize: isPreview ? '0.82rem' : '0.95rem', opacity: 0.75, marginBottom: '0.4rem' }}>
              {message || 'Mantén el equipo encendido y conectado.'}
            </p>
            <p style={{ fontSize: isPreview ? '0.75rem' : '0.9rem', opacity: 0.5, marginTop: isPreview ? '1rem' : '2rem' }}>
